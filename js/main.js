// main.js – Rezepte & Portionsauswahl
...
function scaleIngredients(ingredients, factor) {
  return ingredients.map(item => {
    return item.replace(/(\d+(?:[.,]\d+)?)(\s*(cl|TL|EL|ml|g|Limetten|Stück))/gi, (match, value, unit) => {
      const num = parseFloat(value.replace(',', '.')) * factor;
      const formatted = Number.isInteger(num) ? num.toString() : num.toFixed(2).replace('.', ',');
